...
namespace FamiStudio
{
    ...
    public class PianoRoll : Container
    {
        ...
        internal void UpdateRenderCoords()
        {
            ...
            else if (Platform.IsDesktop && (editMode == EditionMode.Arpeggio || editMode == EditionMode.Envelope))
            {
                var maxValue = 128 / (int)envelopeValueZoom;
                virtualSizeY = (int)(envelopeValueSizeY * maxValue);
            }
        ...
        public void StartEditInstrument(Instrument instrument, int envelope)
        {
            ...
            noteTooltip = "";
            envelopeValueZoom = (envelope == EnvelopeType.Volume && !instrument.IsFds && !(instrument.IsVrc6 && instrument.Vrc6SawFullVolume)) || envelope == EnvelopeType.DutyCycle || envelope == EnvelopeType.N163Waveform ? 2 : 1;
        ...
        private int GetPixelYForEffectValue(int effect, int value, Instrument instrument = null)
        {
            var channel = Song.Channels[editChannel];
            var minValue = Note.GetEffectMinValue(Song, channel, effect);
            var maxValue = Note.GetEffectMaxValue(Song, channel, effect, instrument);
            ...
            TransformNotes(startFrameIdx, startFrameIdx + notes.Length - 1, doTransaction, true, true, (note, idx) =>
            {
                ...
                for (int i = 0; i < Note.EffectCount; i++)
                {
                    if ((pasteFxMask & (1 << i)) != 0 && (!mix || !note.HasValidEffectValue(i) && newNote.HasValidEffectValue(i)))
                    {
                        ...
                        {
                            int clampedEffectValue = Note.ClampEffectValue(Song, channel, i, newNote.GetEffectValue(i), note.Instrument);
        ...
        private void CopyEffectValues(bool text)
        {
            if (editMode == EditionMode.Channel && selectedEffectIdx >= 0 && IsSelectionValid())
            {
                ...
                var prevVal = wantsPreviousValue ? channel.GetLastEffectValue(location, selectedEffectIdx) : Note.GetEffectDefaultValue(Song, channel, selectedEffectIdx);
        ...
        private void ReplaceEnvelopeValues(sbyte[] values, int startIdx)
        {
            ...
            () =>
            {
                ...
                if (!legacySelectMode)
                {
                    ...
                    for (var i = startIdx; i <= startIdx + values.Length - 1; i++)
                        selectedEnvelopeIndices.Add(i);
                }
            });
        }
        ...
        void UpdateChangeEffectValue(int x, int y)
        {
            ...
            if (pattern == null || !pattern.TryGetNoteWithEffectAt(location.NoteIndex, selectedEffectIdx, out note))
            {
            ...
            var max = Note.GetEffectMaxValue(Song, channel, selectedEffectIdx, note.Instrument);
        ...
        public void StartDragVolumeSlide(int x, int y, NoteLocation location)
        {
            ...
            var maxVolume = Note.GetEffectMaxValue(Song, channel, Note.EffectVolume, note.Instrument);
            var ratio = Utils.Clamp(1.0f - (y - headerSizeY) / (float)effectPanelSizeY, 0.0f, 1.0f);
            var volume = (byte)Math.Round(ratio * maxVolume);
        ...
        void StartDragVolumeSlideGizmo(int x, int y, Note note, NoteLocation location)
        {
            var channel = Song.Channels[editChannel];
            var pattern = channel.PatternInstances[location.PatternIndex];
            var offsetY = headerSizeY + effectPanelSizeY - GetPixelYForEffectValue(Note.EffectVolumeSlide, note.VolumeSlideTarget, note.Instrument) - y;
        ...
        void UpdateDragVolumeSlide(int x, int y, bool final)
        {
            ...
            // On mobile, we use gizmos, so let's pretend the drag is happening at the effect value instead of at the gizmo position.
            if (Platform.IsMobile)
                y += headerAndEffectSizeY - GetPixelYForEffectValue(note.VolumeSlideTarget, 0, maxVolume) - captureMouseY;

            var ratio = Utils.Clamp(1.0f - (y - headerSizeY) / (float)effectPanelSizeY, 0.0f, 1.0f);
            note.VolumeSlideTarget = (byte)Math.Round(ratio * maxVolume);
        ...
        private void TransformEnvelopeValues(int startFrameIdx, int endFrameIdx, Func<sbyte, int, sbyte> function, Action postTransform = null)
        {
            if (editMode == EditionMode.Arpeggio)
                App.UndoRedoManager.BeginTransaction(TransactionScope.Arpeggio, editArpeggio.Id);
            else
        ...
        private void ToggleVolumeSlide(NoteLocation location, Note note)
        {
            ...
            if (note.HasVolumeSlide)
                ...
                note.VolumeSlideTarget = (byte)Utils.Clamp(note.Volume + note.Value >= 8 ? -5 : 5, 0, maxVolume);
        ...
        private void EnterEffectValue(int x, int y, NoteLocation location, Note note)
        {
            ...
            var min = Note.GetEffectMinValue(Song, channel, selectedEffectIdx);
            var max = Note.GetEffectMaxValue(Song, channel, selectedEffectIdx, note.Instrument);
            var def = Note.GetEffectDefaultValue(Song, channel, selectedEffectIdx, note.Instrument);
