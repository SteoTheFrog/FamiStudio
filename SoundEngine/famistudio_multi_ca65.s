...
.if FAMISTUDIO_USE_VRC6_SAW_FULL_VOLUME && (FAMISTUDIO_USE_VOLUME_TRACK = 0)
    .error "VRC6 64-step saw volume can only be used when the volume track is enabled too."
.endif
...
famistudio_volume_slide_frac: .res FAMISTUDIO_NUM_VOLUME_SLIDES
.endif
.endif
.endif
...
famistudio_vrc6_saw_full_volume:  .res 1
.endif
.if FAMISTUDIO_USE_PHASE_RESET
...
.if FAMISTUDIO_EXP_FDS
famistudio_fds_master_volume:     .res 1 ; Bit 7 = hold volume, bit 6 = full range volume.
...
@compute_volume:
    ...
    .if FAMISTUDIO_EXP_VRC6 && FAMISTUDIO_USE_VRC6_SAW_FULL_VOLUME && (idx = FAMISTUDIO_VRC6_CH2_IDX)
    ...
    .if FAMISTUDIO_USE_VOLUME_TRACK
        ...
        .if FAMISTUDIO_EXP_VRC6 && FAMISTUDIO_USE_VRC6_SAW_FULL_VOLUME && (idx = FAMISTUDIO_VRC6_CH0_IDX || idx = FAMISTUDIO_VRC6_CH1_IDX)
            cmp #16
            bcc @env_clamped
            lda #15
        @env_clamped:
...
@volume_side_active:
    ...
    ; Refresh A and its flags here unconditionally -- the VRC6 saw check above uses CPX/LDA,
...
famistudio_update_fds_wave:
    ...
    @wave_loop:
        @patch_enable:
            lda #$00 ; Operand patched above every time the wave changes.
